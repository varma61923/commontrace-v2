...
| 6.4 | No mobile / JVM / .NET SDKs | **Not done** | Python only, plus MCP — which is the reason the gap is smaller than it looks: any MCP-capable client reaches the whole tool surface without an SDK. A TypeScript client is the first one worth building. |
