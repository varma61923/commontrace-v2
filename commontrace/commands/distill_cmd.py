...
def _similarity_threshold(value: str) -> float:
    ...
    threshold` reaches here from an argparse CLI flag AND, unvalidated,
    from the customer-facing MCP tool `propose_lessons`'s `similarity`
    parameter (`mcp_server.py`, routed through this exact parser so the two
    surfaces can't drift) -- an agent passing `similarity=0` on a store of a
    few thousand traces would trigger a several-million-comparison medoid
    computation in one tool call, with no warning that 0 means something
    qualitatively different from "loose matching" rather than "looser".
    Rejecting non-positive and above-1 values here (Jaccard similarity is
    never outside [0, 1], so nothing in that range is even meaningful) is
    scoped to the two argument-parsing entry points, not the library
    function real callers may still use directly.
    """
    try:
