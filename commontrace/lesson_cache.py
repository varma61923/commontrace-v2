...
STALENESS IS DETECTED, NEVER ASSUMED. Each entry records the file's
`(mtime_ns, size)`. Every load stats every lesson file -- ~15 ms at 10k, against
the ~36 s of YAML it replaces -- and reparses exactly the files whose stat
changed, plus any that are new. Deleted files leave. So an edit costs one parse,
not N, and the answer is always the store as it is right now. This is the same
freshness contract `commontrace/reference/query.py`'s `check_staleness` uses for
the semantic index, for the same reason: `mcp_server`'s docstring claims lexical
retrieval "cannot go stale", and that claim has to stay literally true.
...
def _load_entries(root: str, reader=None) -> dict[str, dict]:
    """path -> {"mtime_ns", "size", "fm", "terms"} for every lesson file.
...
def load_projected(root: str, reader=None) -> list[tuple[str, dict]]:
    ...
    """
    entries = _load_entries(root, reader=reader)
    # `_lesson_paths` order, not dict order: Python dicts preserve insertion
    # order and `_load_entries` inserts in that same order today, but a
    # future change to it must not silently reorder the corpus out from under
    # `rank_lessons`'s stable-sort tie-breaking.
    return [(p, entries[p]["fm"]) for p in _lesson_paths(root) if p in entries]
