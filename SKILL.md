---
name: commontrace
description: "A+B double-review pattern with sub-agents and loop until conformity (max 3 iterations, then orchestrator arbitration). Spawns a sub-agent A (implementer) + immediate commit + an independent sub-agent B (reviewer). Iterates on gaps. Ideal for architectural code, heavy refactors, CUDA/GPU ports, critical fixes where an independent review adds value. v2 adds a long-term learning mechanism: Alpha (memory retrieval) in Phase 0, Omega (synthesis + lesson proposals) in Phase 10, automatic Lambda validation of the memory backlog in Phase 11. v2.1 adds scalar importance 1-5 on episodes and lessons. v2.2 replaces user validation with an independent Lambda reviewer sub-agent (100% automated workflow). v2.3 adds a semantic attention layer (local embeddings) as an Alpha pre-filter to allow scaling (100+ lessons) without degrading latency or quality. Invocation: `/commontrace <task description + inline success criteria>`. Triggers: `/commontrace`, `do double review`, `commontrace this task`, `launch A+B on ...`."
---
